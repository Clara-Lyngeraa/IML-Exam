...
def load_csv(test_subject, trial, folder, file):
    ...
    file_path = os.path.abspath(f'./data/test_subjects/{test_subject}/{trial}/{folder}/{file}.csv')
...
def load_csv_0(test_subject, folder, filename):
    ...
    Returns:
        data (pd.DataFrame): The loaded CSV file as a pandas DataFrame.
    """
    file_path = os.path.join(os.path.relpath(f'./data/test_subjects/test_subject_0/gaze/' + folder), f'{filename}.csv')#f'data/test_subjects/{test_subject}/gaze/' + folder), f'{filename}.csv')
